...
import { getAllLearnSlugs, getLearnArticleBySlug } from "@/lib/articles/learn";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return createPageMetadata({
    title: `${article.title} | Aether Applied Learn`,
...
export default async function LearnArticlePage({ params }: PageProps) {
  ...
  return (
    <>
      <ArticlePageSchema hub="learn" article={article} />
      <Nav />
          ...
          <article className="mx-auto max-w-[720px]">
            <Reveal>
              <header className="border-b border-ink/10 pb-10">
                <ArticleBreadcrumbs hub="learn" pageTitle={article.title} />
            ...
            <div className="border-t border-ink/10 py-12">
              <Link href="/learn/" className="text-[16px] text-blue-900 hover:underline">
                ← Back to Learn
              </Link>
            </div>
          </article>
